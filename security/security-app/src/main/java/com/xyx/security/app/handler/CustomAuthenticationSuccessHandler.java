...
/**
 * @Author: czw
 * @CreateTime: 2019-09-02 13:43
 * @UpdeteTime: 2019-09-02 13:43
 * @Description:自定义身份验证成功处理器
 * app模块中该类的作用是：我们的其他应用（例如app）,会请求我们的access_token当验证通过后
 * 通过该类，将对应的令牌返回给他们
 * 同时，当app的社交登入时，app会引导用户社交登入，同时qq等服务提供商会将授权码返回给app
 * 此时就需要app携带该授权码请求我们，成功后我们会将qq分配的access_token返回回去
 * 然后app在根据其openId，向我们请求我们的access_token
 */
