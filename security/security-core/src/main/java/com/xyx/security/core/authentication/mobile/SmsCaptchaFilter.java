...
import com.xyx.security.core.validate.CaptchaRepository;
import com.xyx.security.core.validate.code.CaptchaException;
import com.xyx.security.core.validate.code.abstracts.CaptchaProcessor;
...
/**
 * @Author: czw
 * @CreateTime: 2019-08-31 11:17
 * @UpdeteTime: 2019-08-31 11:17
 * @Description:短信验证码过滤器，继承oncePerRequestFilter类后，保证该过滤器 只会调用一次
 */
@Component
@Slf4j
//包含equals和hashcode方法
@Data
//callSuper=false可能导致：有多个类有相同的部分属性，把它们定义到父类中，恰好id（数据库主键）也在父类中，
...
public class SmsCaptchaFilter extends OncePerRequestFilter implements InitializingBean {
	...
	//springSecurity中判断某个路径是否包含某个路径的工具类，例如/usr/**包含了/user/a
