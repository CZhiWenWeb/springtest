...
import com.xyx.shiro.util.common.HexConvertUtil;
...
public class AesCipherUtil {
    ...
    public static String enCrypto(String str) {
        try {
            ...
            throw new ClassCastException("getInstance()方法异常:" + e.getMessage());
        } catch (UnsupportedEncodingException e) {
            logger.error("Base64加密异常:" + e.getMessage());
            throw new ClassCastException("Base64加密异常:" + e.getMessage());
        } catch (InvalidKeyException e) {
            logger.error("初始化Cipher对象异常:" + e.getMessage());
            throw new ClassCastException("初始化Cipher对象异常:" + e.getMessage());
        } catch (IllegalBlockSizeException | BadPaddingException e) {
            logger.error("加密异常，密钥有误:" + e.getMessage());
            throw new ClassCastException("加密异常，密钥有误:" + e.getMessage());
        }
    }
    ...
    public static String deCrypto(String str) {
        try {
            ...
            throw new ClassCastException("Base64解密异常:" + e.getMessage());
        } catch (InvalidKeyException e) {
            logger.error("初始化Cipher对象异常:" + e.getMessage());
            throw new ClassCastException("初始化Cipher对象异常:" + e.getMessage());
        } catch (IllegalBlockSizeException | BadPaddingException e) {
            logger.error("解密异常，密钥有误:" + e.getMessage());
            throw new ClassCastException("解密异常，密钥有误:" + e.getMessage());
        }
    }
}
